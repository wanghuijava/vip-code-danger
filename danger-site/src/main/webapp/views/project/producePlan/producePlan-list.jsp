...
<html>
	...
	<body>
		<form id="list-form" action="/project/producePlan/find">
			<div class="layout layout-full">
			    <div class="layout layout-search">
	                ...
	                <div class="btn-grp ibm">
	                    <button type="button" class="btn btn-normal" data-href="${siteUrl }/project/producePlan/addPage">
			    ...
			    <div class="layout layout-search-result">
			        ...
			        <div x-scrollable class="layout layout-table-body cliptb">
						...
						<tr class="
		                    ...
		                    <td class="anchor" title="{{item.workName}}"><a href="${siteUrl }/project/producePlan/viewPage/{{item.id}}">{{item.workName}}</a></td>
			                ...
			                <td class="txt-ac expe">
								{{if item.checkFlag==0}}
								<label class="label label-red <c:if test="${user.typeCode=='2'}">anchor</c:if>" <c:if test="${user.typeCode=='2'}"> title="点击上报" data-post-url="/project/producePlan/report" data-confirm="是否确认上报？" data-post-param='{"id":"{{item.id}}"}'</c:if> >{{item.checkFlagStr}}</label>
								{{else}}
		                    ...
		                    <td class="txt-ac expe">
								...
								<button type="button" class="btn btn-green" data-href="${siteUrl }/project/producePlan/editPage/{{item.id}}">
									<span class="btn-otr">
										<span class="btn-inr"><i class="icon icon18 icon-edit"></i>编辑</span>
									</span>
								</button>
								<button type="button" class="btn btn-red" data-post-url="/project/producePlan/delete" data-confirm="" data-post-param='{"id":"{{item.id}}"}'>
									<span class="btn-otr">

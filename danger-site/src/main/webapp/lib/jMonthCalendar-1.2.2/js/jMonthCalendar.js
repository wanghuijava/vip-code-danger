...
(function($) {
	...
	var defaults = {
			...
			onMonthChanging: function(dateIn,e) { return true; },
			onMonthChanged: function(dateIn,e) { return true; },
			onEventLinkClick: function(event,e) { return true; },
			onEventBlockClick: function(event,e) { return true; },
			onEventBlockOver: function(event,e) { return true; },
			onEventBlockOut: function(event,e) { return true; },
			onDayLinkClick: function(date,e) { return true; },
			onDayCellClick: function(date,e) { return true; },
			onDayCellDblClick: function(dateIn,e) { return true; },
			onEventDropped: function(event, newDate,e) { return true; },
			locale: {
		...
		do {
			...
			for (var i=0; i<7; i++) {
				...
				var dateLink = jQuery('<div class="DateLabel"><a href="">' + _currentDate.getDate() + '</a></div>').click(function(e) {
                    defaults.onDayLinkClick(new Date($(this).parent().attr("date")),e);
                    e.stopPropagation();
                });
				
				var dateBox = jQuery("<td></td>").attr(atts).append(dateLink).dblclick(function(e) {
                    defaults.onDayCellDblClick(new Date($(this).attr("date")),e);
                    e.stopPropagation();
                }).click(function(e) {
                	//传递事件 added by yk
					defaults.onDayCellClick(new Date($(this).attr("date")),e);
                    e.stopPropagation();
				});
				...
				if (defaults.dragableEvents) {
                    dateBox.droppable({
                        ...
                        drop: function(e, ui) {
                            ...
                            defaults.onEventDropped(event, $(this).attr("date"),e);
							return false;
                        }
                    });
                }
				...
				if(sDt) {
					if ((sDt >= _beginDate) && (sDt <= _endDate)) {			
						...
						link.click(function(e) {
							defaults.onEventLinkClick(ev,e);
							e.stopPropagation();
						});
						...
						event.click(function(e) { 
							defaults.onEventBlockClick(ev,e); 
							e.stopPropagation(); 
						});
						event.hover(function(e) { defaults.onEventBlockOver(ev,e); }, function(e) { defaults.onEventBlockOut(ev,e); })

...
<body>
	<div class="content-menu ib-a blue line-x">
		<a href="${adminUrl}/admin/basedata/district/list"> <em>管理行政区划</em>
		</a>
		<span>|</span>
		<a href="javascript:;" class="on"> <em>编辑行政区划</em>
		</a>
	</div>
		...
		<div class="pad-10">
			...
			<div class="col-tab">
				...
				<div id="div_setting_1" class="contentList pad-10">
					<form id="editDistrictForm" class="form-horizontal" action="${adminUrl}/admin/basedata/district/update/${district.distCode}" method="post">
						<div class="control-group">
							<label class="control-label">上级行政区划</label>
							<div class="controls">
								<select class="span2" name="parentDistrictCode" id="parentDistrict">
									<option value="" <c:if test="${empty district.parentCode}">selected</c:if>>≡ 作为一级节点 ≡</option>		
									<!-- 一级节点 -->		
									<c:forEach var="select" items="${districtList}">
										<option value="${select.distCode}" <c:if test="${district.parentCode == select.distCode}">selected</c:if>>${select.distName}</option>
										...
										<c:if test="${!empty select.children}">
											<c:forEach var="selectChild1" items="${select.children}" varStatus="childIndex1">
												...
												<c:when test="${childIndex1.index == fn:length(select.children) - 1}">
													<option value="${selectChild1.distCode}" <c:if test="${district.parentCode == selectChild1.distCode}">selected</c:if>>&nbsp;└ ${selectChild1.distName}</option>
												</c:when>
												<c:otherwise>
													<option value="${selectChild1.distCode}" <c:if test="${district.parentCode == selectChild1.distCode}">selected</c:if>>&nbsp;├ ${selectChild1.distName}</option>
												</c:otherwise>
						...
						<div class="control-group">
							...
							<div class="controls">
								<span class="input-large uneditable-input">${district.distCode}</span>
								<input type="hidden" id="distCode" name="distCode" value="${district.distCode}" maxlength="20" />
								<font color="red"><span id="distCodeValidate">*</span></font>
								<!-- span class="help-inline"><div id="distCodeTip" class="onShow">请输入行政区划编码</div></span>
						...
						<div class="control-group">
							...
							<div class="controls">
								<input type="text" id="distName" name="distName" value="${district.distName}" maxlength="20" />
								<font color="red"><span id="distNameValidate">*</span></font>
								<!-- span class="help-inline"><div id="distNameTip" class="onShow">请输入行政区划全称</div></span>
						...
						<div class="control-group">
							...
							<div class="controls">
								<input type="text" id="distShortName" name="distShortName" value="${district.distShortName}" maxlength="20" />
						...
						<div class="control-group">
							...
							<div class="controls">
								<input type="text" id="distLongitude" name="distLongitude" value="${district.longitude}" maxlength="20" />
						...
						<div class="control-group">
							...
							<div class="controls">
								<input type="text" id="distLatitude" name="distLatitude" value="${district.latitude}" maxlength="20" />
						...
						<div class="control-group">
							...
							<div class="controls">
								<input type="text" id="distNotes" name="distNotes" value="${district.distNotes}" maxlength="50" />
							</div>
						</div>
					</form>
				</div>
...
var CheckAndSubmit = function(){
	...
	if(distCode==null || distCode==""){
		$("#distCodeValidate").html("行政区划代码不能为空");
		$("#distCode").focus();
		return false;
	...
	if(distName==null || distName==""){
		$("#distNameValidate").html("行政区划全称不能为空");
		$("#distName").focus();
		return false;
	}
	
	if(distShortName==null || distShortName==""){
		$("#distShortNameValidate").html("行政区划简称不能为空");
		$("#distShortName").focus();
		return false;
	}
	
	if(distLongitude==null || distLongitude==""){
		$("#distLongitudeValidate").html("行政区划精度不能为空");
		$("#distLongitude").focus();
		return false;
	}
	
	if(distLatitude==null || distLatitude==""){
		$("#distLatitudeValidate").html("行政区划纬度不能为空");
		$("#distLatitude").focus();
		return false;
	}
	
	$("#editDistrictForm").submit();
}

 ...
 */
@Path("project")
public class WorkPlanImportController {
	@Autowired
	...
	private String saveDatas(Sheet sheet) {
		...
		String[] columnNames = {"公司名称", "危险作业项目名称", "作业级别", "工程概况", "计划实施时间（格式：2016-01-01）", "作业审批人", "作业复核人", "现场指挥"};
		...
		if(!supFlag){
			columnNames = new String[]{"危险作业项目名称", "作业级别", "工程概况", "计划实施时间（格式：2016-01-01）", "作业审批人", "作业复核人", "现场指挥"};
...
//				state = "1";
			...
			if(supFlag){
				...
				workPlan.setOrgName(orgName);
				workPlan.setOrgCode(CodesUtils.getOrgCodeByOrgName(orgName));
			}else{
				workPlan.setOrgName(orgNameT);
				workPlan.setOrgCode(orgCodeT);
			}
			...
			if(StringUtils.isNotEmpty(workLevel)){
				workPlan.setWorkLevel(workLevel);
			}
			...
			workPlan.setExecuteDate(DateUtil.getDate(executeDate));
			//下级要验证计划的时间
			if(!supFlag && (workPlan.getExecuteDate().before(begin) || workPlan.getExecuteDate().after(end))){
